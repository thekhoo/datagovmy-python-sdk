...
All query parameters from the [data.gov.my API](https://developer.data.gov.my/request-query) are supported as keyword arguments.

### Limit results

```python
# Get only the first 5 records
data = client.data_catalogue.get_dataset_as_json(id="population_malaysia", limit=5)
```
...
data = client.opendosm.get_dataset_as_json(
    id="cpi_2d_category",
...
| `meta` | `true` | Include metadata in response |
